...
function hook(P) {
    ...
    try {
      if (src && (src instanceof ArrayBuffer || ArrayBuffer.isView(src)) && window.__capBuf.length < 900) {
        ...
        if (u8.length >= 16) {
          ...
          window.__capBuf.push({seq: window.__bufSeq++, id: oid(this, 'B'), target: String(target), len: u8.length, b64: btoa(s)});
        }
      }
    } catch (e) {}
  ...
  pr.bindBuffer = function(target, buf) {
    try { curBuf[target] = oid(buf, 'B'); } catch (e) {}
  ...
  pr.shaderSource = function(shader, src) {
    try {
      if (typeof src === 'string' && window.__capShaders.length < 60)
        window.__capShaders.push({kind: this instanceof P && P === window.WebGLRenderingContext ? 'gl1' : 'gl2', src: src});
    } catch (e) {}
  ...
  pr.getUniformLocation = function(prog, name) {
    try {
      ...
      if (loc) {
        if (!progUniforms[pid]) progUniforms[pid] = {};
        progUniforms[pid][String(loc)] = name;
      }
      return loc;
  ...
  ['uniform1i', 'uniform1f', 'uniform2f', 'uniform3f', 'uniform4f',
   'uniform1fv', 'uniform2fv', 'uniform3fv', 'uniform4fv'].forEach(fn => {
    const o = pr[fn];
    if (!o) return;
    pr[fn] = function(loc) {
      try {
        if (window.__capUniforms.length < 20000) {
          const name = (progUniforms[curProg] && progUniforms[curProg][String(loc)]) || ('uloc' + loc);
          const args = [];
          ...
          window.__capUniforms.push({p: curProg, f: fn, n: name, v: vals});
        }
      } catch (e) {}
  ...
  pr.drawElements = function(mode, count, type, offset) {
    try {
      if (window.__capDraws.length < 4000)
        window.__capDraws.push({m: String(mode), c: count, t: String(type), o: offset,
  ...
  pr.drawArrays = function(mode, first, count) {
    try {
      if (window.__capDraws.length < 4000)
        window.__capDraws.push({m: String(mode), c: count, first: first, arr: curBuf[34962],
                                prog: curProg, a: snapshotAttribs()});
    } catch (e) {}
...
def pull(page):
    ...
    n_uni = page.evaluate('window.__capUniforms.length')
    print(f'[pull] shaders={n_sh} uniforms={n_uni}', flush=True)
    shaders = page.evaluate('window.__capShaders')
    with open(os.path.join(OUT, 'shaders.json'), 'w') as f:
        json.dump(shaders, f)
    with open(os.path.join(OUT, 'uniforms.json'), 'w') as f:
        json.dump(page.evaluate('window.__capUniforms'), f)
