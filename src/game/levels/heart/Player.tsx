"use client";
/**
 * Nano-robot player rig (spec §12/§33/§34): POV + CHASE cameras, damped
 * propulsion, vessel-wall collision, boost FOV kick, scan spotlight.
 */
...
export function createPlayerRefs(): PlayerRefs {
  return {
...
interface Props {
  ...
  hitWallRef: React.MutableRefObject<number>; // impact magnitude per frame
  quality: "LOW" | "MEDIUM" | "HIGH";
}

export function Player({ player, input, flowRef, beatRef, hitWallRef, quality }: Props) {
  ...
  useFrame((state, rawDt) => {
    ...
    _dir.set(0, 0, 0);
    if (playing) {
      // camera-relative basis from yaw/pitch
    ...
    // ---- vessel projection + wall collision ----
    // local search around previous t for closest curve param
    let bestT = player.t;
    let bestD = Infinity;
    for (let k = -6; k <= 6; k++) {
      const t = THREE.MathUtils.clamp(player.t + k * 0.002, 0, 1);
      vesselCurve.getPointAt(t, _center);
    ...
    player.t = bestT;
    vesselCurve.getPointAt(bestT, _center);
    _offset.copy(player.pos).sub(_center);
    const radius = VESSEL_BASE_RADIUS * vesselRadiusAt(bestT, flowRef.current);
  ...
  return (
    <>
      <spotLight
        ...
        position={[0, 0, 0]}
      />
      {/* scan reticle ring in world space (subtle, in front of camera) */}
