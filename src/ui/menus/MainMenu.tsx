"use client";
/**
 * MAIN MENU (spec §29): cinematic 3D backdrop handled by MenuScene; this is
 * the typographic + action layer. One focal point, one primary action.
 */
...
export function MainMenu() {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-30 flex flex-col">
      ...
      <div className="mt-auto w-full px-6 pb-[max(env(safe-area-inset-bottom),10vh)] sm:px-12">
        <div
        ...
        >
          ...
          <p className="mt-4 font-mono text-[11px] tracking-[0.3em] text-white/60 sm:text-xs">
          ...
          <div className="pointer-events-auto mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
            <button
              className="group relative overflow-hidden rounded-sm border border-cyan-300/60 bg-cyan-400/10 px-9 py-4 font-mono text-sm tracking-[0.3em] text-cyan-100 transition-all duration-300 hover:bg-cyan-300/25 hover:shadow-[0_0_40px_rgba(45,217,232,0.35)]"
          ...
          {discoveries.length > 0 && (
            <p className="mt-6 font-mono text-[10px] tracking-[0.25em] text-white/40">
              ANATOMY JOURNAL: {discoveries.length} DISCOVER{discoveries.length === 1 ? "Y" : "IES"}
            </p>
          )}
        </div>
      </div>
