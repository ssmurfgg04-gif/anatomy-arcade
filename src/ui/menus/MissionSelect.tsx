"use client";
/**
 * MISSION SELECT (spec §30): three mission nodes, asymmetric layout, status.
 */
...
const MISSIONS: {
  ...
  { id: "viral", num: "02", name: "VIRAL INVASION", desc: "Alveolar region. Identify infected cells. Assist immunity.", status: "READY", accent: "#2DD9E8" },
  { id: "brain", num: "03", name: "BRAIN MISSION", desc: "Neural network. Reconnect pathways. Restore the signal.", status: "READY", accent: "#8f6fd8" },
];
        ...
        <div className="mt-6 flex flex-col gap-4">
          {MISSIONS.map((m) => (
            ...
            >
              ...
              <span className="flex items-center pr-5 font-mono text-[9px] tracking-[0.3em]" style={{ color: m.status === "READY" ? "#2DD9E8" : "rgba(255,255,255,0.3)" }}>
                {m.status === "READY" ? "▸ READY" : "LOCKED"}
              </span>
            </button>
          ))}
        </div>
        <div className="mt-10 grid gap-3 border-t border-white/10 pt-6 pb-8 sm:grid-cols-3">
          {[
